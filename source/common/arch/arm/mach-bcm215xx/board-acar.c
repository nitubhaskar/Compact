...
#define PMEM_ADSP_SIZE (1024 * 1024 * 16)
#endif
...
#define I2C_PRIMARY_CAM_SLAVE_ADDRESS		0x7A   /*mt9t111*/
...
#define GPIO16_AS_GPIO (~(GPIO16_AS_PWM0))
	...
	case SYSCFG_SDHC3:
		...
		printk(KERN_INFO"[WYQ] SDHC3 config =0x%x\n",val2);
		if (op == SYSCFG_INIT) {
			/* Offset for IOCR0 = 0x00 */
			/* MSPRO_SD3 = 0x3 */
			writel((readl(ADDR_SYSCFG_IOCR0) &
		...
		val2 = readl(ADDR_SYSCFG_IOCR4);
		printk(KERN_INFO"[WYQ] SDHC3 after config =0x%x\n",val2);
		break;
#ifdef CONFIG_USB_DWC_OTG
	case SYSCFG_USB:
		if (op == SYSCFG_DISABLE)
