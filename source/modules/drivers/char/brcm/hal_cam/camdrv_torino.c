...
#if defined (CONFIG_BCM_DUAL_CAM) ////////////////for dual camera
  ...
  //  char temp[8];
    ...
    { 
      ...
      //  Log_DebugPrintf(LOGID_SYSINTERFACE_HAL_CAM, "cam_WriteAutoIncrement1(): i2c_sub_adr = 0x%x");
        ...
        strcat(dstr, ")");        
        Log_DebugPrintf(LOGID_SYSINTERFACE_HAL_CAM,"%s", dstr);        
#endif       
    }
    else
    {
        //HalcamTraceErr("cam_WriteAutoIncrement1(): sCamI2cStatus ERROR: ");
    }
}
...
/*
	...
	if (!cam_regulator_i || IS_ERR(cam_regulator_i)) {
		HalcamTraceErr( "luisa_[cam_vddi]No Regulator available");
		rc = -EFAULT;
		return rc;
	}
*/	
	...
	if (!cam_regulator_a || IS_ERR(cam_regulator_a)) {
		HalcamTraceErr( "luisa_[cam_vdda]No Regulator available");
		rc = -EFAULT;
		return rc;
	}
	cam_regulator_c = regulator_get(NULL, "cam_vddc");
	if (!cam_regulator_c|| IS_ERR(cam_regulator_c)) {
		HalcamTraceErr( "luisa_[cam_vddc]No Regulator available");
		rc = -EFAULT;
		return rc;
	}
	return 1;
...
static HAL_CAM_Result_en_t Init_torino_sensor(CamSensorSelect_t sensor)
{
	...
	if(iRet<0)
	{
      ...
      //reset settings	
       ...
       Drv_Effect=NULL;
       Drv_DTPmode= NULL;
        HalcamTraceDbg("Init_torino_sensor end ");
...
UInt8  CAMDRV_CheckEXP(UInt8 mode)
{
    ...
    CamacqExtReadI2c( pstSensor->m_pI2cClient, Write_Exptime3,1, &Read_Exptime3,1);



    HalcamTraceDbg("CheckEXP %x, %x, %x, %x",Read_Exptime0,Read_Exptime1,Read_Exptime2,Read_Exptime3);

    
    Exptime  = (Read_Exptime0 <<24) |0x000000;
    Exptime |= (Read_Exptime1 <<16) | 0x0000;
    Exptime |= (Read_Exptime2 <<8) | 0x00;
    Exptime |= (Read_Exptime3);
...
//           gv_checkEXPtime = ((float) Exptime/24000000.0f);
    ...
    {
		gv_checkEXPtime = 27000000/Exptime;
...
void  CAMDRV_CheckISO(void)
{
	...
	struct stCamacqSensor_t* pstSensor = NULL;
       ...
       U8 Page0[2] = {0x03,0x20};
       U8 Write_Exptime0 = 0x50,Read_Exptime0=0;
       U32 sensor= 0;//Rear Camera
...
static HAL_CAM_Result_en_t CAMDRV_SetSensorParams( CAM_Parm_t parm,CamSensorSelect_t sensor)
{
    ...
    HalcamTraceDbg("after GetCamacqSensorManager, sensor = %d", sensor);
    ...
    HalcamTraceDbg("after GetSensor");
