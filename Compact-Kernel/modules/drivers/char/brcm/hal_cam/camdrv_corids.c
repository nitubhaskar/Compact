...
#include "camacq_sr200pc10_ltn.h"
#else
...
// [[ mhoon.chae@LTN_MM support anti-banding
#if defined(CONFIG_LTN_COMMON)
extern int camera_antibanding_get();
...
static CAM_Sensor_Supported_Params_t CamPrimaryDefault_st =
{
	...
	"SAMSUNG",
#if defined(CONFIG_CORIDS_L_LTN_COMMON)
	"GT-S5302L",
#elif defined(CONFIG_CORIDS_B_LTN_COMMON)
	"GT-S5302B",
#else
	"GT-S5302", 
#endif
...
static HAL_CAM_Result_en_t Init_corids_sensor(CamSensorSelect_t sensor)
{
	HAL_CAM_Result_en_t result = HAL_CAM_SUCCESS;
	...
	if(iRet<0)
	{
      ...
      //reset settings	
       ...
       Drv_Mode = -1;
        ...
        HalcamTraceDbg("Init_corids_sensor end \n");
...
// [[ mhoon.chae@LTN_MM support anti-banding
#if defined(CONFIG_LTN_COMMON)
#define MAX_TABLE_INDEX 7000 // It's possible to increase

int getsizeRow2D(const U8 pp[][2])
{
	int indexRow = 0;
	...
	for( indexRow = 0; indexRow < MAX_TABLE_INDEX; indexRow++)
	{
		if( (*(*(pp+indexRow))	== 0xff) && (*(*(pp+indexRow) + 1) == 0xff) )
		{
			indexRow++;
			break;
		}
	...
	if (indexRow >= MAX_TABLE_INDEX)
	{
		indexRow = 0;
	}

	printk(KERN_ERR"[LTN] row_size is [%d]", indexRow);

	return indexRow;
}

#define COPY_FROM_60HZ_TABLE(TABLE_NAME) \
{\
	int rowSize = 0, rowSize2 =0;\
	rowSize = getsizeRow2D(TABLE_NAME);\
	rowSize2 = getsizeRow2D(TABLE_NAME##_60hz);\
	errTableIndex--;\
	if((rowSize2 == rowSize) && (rowSize > 0))\
		{memcpy (TABLE_NAME, TABLE_NAME##_60hz, rowSize*sizeof(U8)*2);\
		printk(KERN_ERR"[LTN] memcpy Done! rowSize = [%d], rowSize2 = [%d]", rowSize, rowSize2);}\
	else \
	{printk(KERN_ERR"[LTN] memcpy error! errTableIndex = [%d]", errTableIndex);}\
}

int CAMDRV_SetTableFor60hzAntibanding(void)
{
	static int errTableIndex = 0;
	errTableIndex = 0;	//init

	//[+ ltn_cam_to_do : setting table
	COPY_FROM_60HZ_TABLE(reg_main_init);
	COPY_FROM_60HZ_TABLE(reg_main_preview);
	COPY_FROM_60HZ_TABLE(reg_main_camcorder);
	COPY_FROM_60HZ_TABLE(reg_main_fps_fixed_15);
	COPY_FROM_60HZ_TABLE(reg_main_scene_auto);
	COPY_FROM_60HZ_TABLE(reg_main_scene_night);
	COPY_FROM_60HZ_TABLE(reg_main_scene_night_dark);
	COPY_FROM_60HZ_TABLE(reg_main_scene_landscape);
	COPY_FROM_60HZ_TABLE(reg_main_scene_indoor);
	COPY_FROM_60HZ_TABLE(reg_main_scene_sunset);
	COPY_FROM_60HZ_TABLE(reg_main_scene_sunrise);
	COPY_FROM_60HZ_TABLE(reg_main_scene_fallcolor);
	COPY_FROM_60HZ_TABLE(reg_main_scene_candlelight);
	COPY_FROM_60HZ_TABLE(reg_main_scene_againstlight);

#if defined(LTN_CAM_DEBUG)	//latin_mm:jyc for debug
	{
		int count, ii;

		printk(KERN_ERR"[LTN] Display setting table ###");

		//HalcamTraceErr("[LTN] reg_main_preview : [%d]", reg_main_preview);
		//HalcamTraceErr("[LTN] reg_main_preview : [%d]", &reg_main_preview[0][0]);

		count = getsizeRow2D(reg_main_preview);

		for( ii = 0 ; ii < count ; ii++)
		{
			printk(KERN_ERR"{0x%02x, 0x%02x},", reg_main_preview[ii][0], reg_main_preview[ii][1]);
		}
	}
#endif
	return 0;
...
HAL_CAM_Result_en_t CAMDRV_SetLtnAntiBanding(void)
{
	int result = 0;
	int value = 0;

	value = camera_antibanding_get();
	printk(KERN_ERR"[LTN] antibanding value is %d", value);

	if(CAM_ANTIBANDING_60HZ == value) {
		result = CAMDRV_SetTableFor60hzAntibanding();
		if(result < 0)
		{
			printk(KERN_ERR"[LTN] CheckTableSize fail! Error_table is [%d]", result);
			return HAL_CAM_ERROR_INTERNAL_ERROR;
		}
	}
	return HAL_CAM_SUCCESS;
...
// [[ mhoon.chae@LTN_MM support anti-banding
	...
	if(sensor == 0)
	{
		printk(KERN_ERR"[LTN] CAMDRV_Wakeup :  CAMDRV_SetLtnAntiBanding\r\n");
		result = CAMDRV_SetLtnAntiBanding();
		if(result != HAL_CAM_SUCCESS)
		{
			printk(KERN_ERR"[LTN] Cori_sensor_set_60hz_antibanding fail! Error = %d ", result);
			return result;
		}
	}
#endif //ltn_cam : support anti-banding hmin84.park 2011.12.08
	result = Init_corids_sensor(sensor);
	 ...
	 HalcamTraceDbg("CAMDRV_GetESDValue() ReadValue2 = 0x%x ",ReadValue);
	  ...
	  if(ReadValue!=0xaa)	
	  {
		HalcamTraceErr("ESD ERROR[3]!!!!");
          ...
          CamacqExtReadI2c( pstSensor->m_pI2cClient, Addr4,1, &ReadValue,1);
	 HalcamTraceDbg("CAMDRV_GetESDValue() ReadValue4 = 0x%x ",ReadValue);
	  if(ReadValue!=0xaa)	
	  {
		HalcamTraceErr("ESD ERROR[4]!!!!");
	       *esd_value = TRUE;
	       return result;
	  }

        CamacqExtReadI2c( pstSensor->m_pI2cClient, Addr5,1, &ReadValue,1);
	  HalcamTraceDbg("CAMDRV_GetESDValue() ReadValue5 = 0x%x ",ReadValue);
	  if(ReadValue!=0xaa)	
	  {
		HalcamTraceErr("ESD ERROR[5]!!!!");
	       *esd_value = TRUE;
	       return result;
	  }
