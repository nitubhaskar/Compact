...
static CAM_Sensor_Supported_Params_t CamSecondaryDefault_st =
{
	...
	{
		{640,480}//swsw_640,480  //What kind of resolution sensor can support for video resolution
	}, 
...
static HAL_CAM_ConfigCaps_st_t CamSecondaryCfgCap_st = 
{
    ...
    // CAMDRV_DATA_MODE_S *stills_mode
   {
        ...
        640,//swsw_320                           // unsigned short max_width;   Maximum width resolution
        480, //swsw_240                          // unsigned short max_height;  Maximum height resolution         
        0,                              // UInt32                data_size;                //Minimum amount of data sent by the camera
...
static CamIntfConfig_CCIR656_st_t CamSecondaryCfg_CCIR656_st = {
	...
	CSL_CAM_SYNC_ACTIVE_LOW,//HIGH			///< UInt32 vsync_polarity; 		   default)ACTIVE_LOW/ACTIVE_HIGH:		  Vsync active	
	...
	CSL_CAM_CLK_EDGE_POS,//swsw_POS				///< UInt32 data_clock_sample;		(default)RISING_EDGE/FALLING_EDGE:		Pixel Clock Sample edge
...
static void  CAMDRV_CheckModeChange(void);
...
//***************************************************************************
//
// Function Name:  vt_SensorWriteI2c
//
...
static HAL_CAM_Result_en_t Init_luisa_sensor(CamSensorSelect_t sensor)
{
	...
	printk(KERN_ERR"write sr030pc30_init0\n");
...
UInt8  CAMDRV_CheckEXP(UInt8 mode)
{
	...
	{
	printk(KERN_ERR"CAM_PREVIEW_else\r\n");
    	    //none
	}
	...
	{
	printk(KERN_ERR"!CAM_SHUTTER_SPEED \r\n");
...
printk(KERN_ERR"CAM_SHUTTER_SPEED \r\n");
	...
	{ printk(KERN_ERR"CAM_SHUTTER_SPEED_else \r\n");
...
//           gv_checkEXPtime = ((float) Exptime/24000000.0f);
      ...
      printk(KERN_ERR"gv_checkEXPtime: %d, gv_checkEXPtime2 : %d ",gv_checkEXPtime, gv_checkEXPtime2);
        ...
        if(mode == CAM_SHUTTER_SPEED)/*shutter speed */
        {
	      gv_checkEXPtime = 12000000/Exptime;
...
static HAL_CAM_Result_en_t CAMDRV_SetSaturation(CamSaturation_t saturation, CamSensorSelect_t sensor)
{
	...
	HAL_CAM_Result_en_t result = HAL_CAM_SUCCESS;
	printk(KERN_ERR"CAMDRV_SetSaturation() called,Empty!! \r\n");
    return result;
}
...
static HAL_CAM_Result_en_t CAMDRV_SetSharpness( CamSharpness_t  sharpness, CamSensorSelect_t sensor)
{
	...
	HAL_CAM_Result_en_t result = HAL_CAM_SUCCESS;
	printk(KERN_ERR"CAMDRV_SetSharpness() called,Empty!!\r\n");
    return result;
}
...
static HAL_CAM_Result_en_t CAMDRV_SetContrast(CamContrast_t contrast, CamSensorSelect_t sensor)
{
	...
	HAL_CAM_Result_en_t result = HAL_CAM_SUCCESS;
	printk(KERN_ERR"CAMDRV_SetContrast() called,Empty!!!  \r\n");
    return result;
}
...
static HAL_CAM_Result_en_t CAMDRV_SetSensitivity( CamSensitivity_t iso, CamSensorSelect_t sensor)
{
    ...
    printk(KERN_ERR"CAMDRV_SetSensitivity() called : Empty!! \r\n");
    return result;
}
...
static HAL_CAM_Result_en_t CAMDRV_SetDTPmode(unsigned int testmode,CamSensorSelect_t sensor)
{
	...
	{ 
	        msleep(100);
            printk(KERN_ERR"swsw_reg_sub_dtp_on \r\n");
        ...
        {   printk(KERN_ERR"swsw_reg_sub_dtp_off \r\n");
...
static HAL_CAM_Result_en_t CAMDRV_GetSensorValuesForEXIF( CAM_Sensor_Values_For_Exif_t *exif_parm,CamSensorSelect_t sensor)
{
    ...
    }
   {
    ...
    strcpy(exif_parm->maxLensAperture,(char *)"297/100" );//2.8
    strcpy(exif_parm->lensFocalLength,(char *)"130/100" );
...
static void  CAMDRV_CheckModeChange(void)
{
	...
	printk(KERN_ERR"CAMDRV_CheckModeChange() called Empty\r\n");
}
