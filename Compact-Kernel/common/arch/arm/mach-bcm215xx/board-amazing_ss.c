...
*
*	@file	arch/arm/mach-bcm215xx/board-amazing_ss.c
*
...
#define BATT_LEVEL5_VOLT	3940
#define BATT_LEVEL4_VOLT	3850
#define BATT_LEVEL3_VOLT	3760
...
static struct i2c_board_info __initdata bcm21553_cam_i2c_board_info[] = {
        {I2C_BOARD_INFO("cami2c_main", (0xAC)>>1) ,
...
#if 1//for amazing AF swsw


	static struct regulator_consumer_supply dldo5_consumers[] = {
	{
		.dev = NULL,
		.supply = "cam_af",
	},
...
static struct regulator_init_data dldo5_init_data = {
	.constraints = {
		.min_uV = 2800000,
		.max_uV = 2800000,
		.valid_ops_mask = REGULATOR_CHANGE_VOLTAGE |
	...
	.num_consumer_supplies = ARRAY_SIZE(dldo5_consumers),
	.consumer_supplies = dldo5_consumers,
};
...
struct regulator_consumer_supply d2041_consumers[D2041_NUMBER_OF_REGULATORS] = {
	...
	[D2041_LDO_10] = REGULATOR_SUPPLY("sim1_vcc", NULL),		
	[D2041_LDO_11] = REGULATOR_SUPPLY("sim_vcc", NULL),
	[D2041_LDO_13] = REGULATOR_SUPPLY("sd_a_vdd", NULL),
	[D2041_LDO_14] = REGULATOR_SUPPLY("touch_vcc_2.9v", NULL),	
	[D2041_LDO_15] = REGULATOR_SUPPLY("vib_vcc", NULL),
	[D2041_LDO_16] = REGULATOR_SUPPLY("cam_af", NULL),
	[D2041_LDO_18] = REGULATOR_SUPPLY("touch_vcc_3.3v", NULL),
...
struct d2041_platform_data d2041_pdata = {	
	...
	.regl_map = {
        ...
        D2041_MCTL_MODE_INIT(D2041_LDO_10, 0x22, D2041_REGULATOR_LPM_IN_DSM),					// VSIM2_3.0V
        D2041_MCTL_MODE_INIT(D2041_LDO_11, 0x22/*0x66*/, D2041_REGULATOR_LPM_IN_DSM),	// 0x11		// VSIM1_3.0V
        ...
        D2041_MCTL_MODE_INIT(D2041_LDO_14, 0x44, D2041_REGULATOR_OFF_IN_DSM),	// 0x22		// TOUCH_2.9V
        D2041_MCTL_MODE_INIT(D2041_LDO_15, 0x00, D2041_REGULATOR_MAX),			// 0xAA		// VIB
        D2041_MCTL_MODE_INIT(D2041_LDO_16, 0x00, D2041_REGULATOR_OFF_IN_DSM),	// 0xAA		// cam_af
        D2041_MCTL_MODE_INIT(D2041_LDO_17, 0x44, D2041_REGULATOR_MAX),			// 0x22		// VBBD
        D2041_MCTL_MODE_INIT(D2041_LDO_18, 0x44, D2041_REGULATOR_OFF_IN_DSM),				// TOUCH_3.3V
...
static struct i2c_board_info __initdata athenaray_i2cgpio5_board_info[] = {
#if 1// sehyoung	
	{
...
#endif
	i2c_register_board_info(0x8, athenaray_i2cgpio5_board_info,
				ARRAY_SIZE(athenaray_i2cgpio5_board_info));
}
...
int board_sysconfig(uint32_t module, uint32_t op)
{
	...
	case SYSCFG_LCD:
		if(op == SYSCFG_ENABLE) {
                	writel(readl(ADDR_SYSCFG_IOCR4) &
				(0xFF4FFFFF), ADDR_SYSCFG_IOCR4);			
			writel(readl(ADDR_SYSCFG_IOCR6) &
				~(SYSCFG_IOCR6_GPIO25_24_MUX),
				ADDR_SYSCFG_IOCR6);
			writel(readl(ADDR_SYSCFG_IOCR2) |
			...
			writel((readl(ADDR_SYSCFG_IOCR0) &	~((SYSCFG_IOCR0_LCD_CTRL_MUX | SYSCFG_IOCR0_MPHI_MUX) | SYSCFG_IOCR0_LCDD1_LCDD15_MUX)), ADDR_SYSCFG_IOCR0);
			writel(readl(ADDR_GPIO_GPIPEN1) | (0x00000001), ADDR_GPIO_GPIPEN1);
			writel(readl(ADDR_GPIO_GPIPUD1) | (0x00000001), ADDR_GPIO_GPIPUD1); 
			writel(readl(ADDR_GPIO_GPIPEN0) | (0x00080000), ADDR_GPIO_GPIPEN0);
			writel(readl(ADDR_GPIO_GPIPUD0) | (0x00080000), ADDR_GPIO_GPIPUD0); 			
			}
		else {
			...
			writel((readl(ADDR_SYSCFG_IOCR0) &
				~((SYSCFG_IOCR0_LCD_CTRL_MUX | SYSCFG_IOCR0_MPHI_MUX) | SYSCFG_IOCR0_LCDD1_LCDD15_MUX)),
				ADDR_SYSCFG_IOCR0);
		}
		break;
 ...
 -----------------------------------------------------------
 GPIO 52    CAM_5M_STBY       enabled (Pull-down)      low out
 GPIO 53    CAM_3M_RST        enabled (pull-down)      low out
 GPIO 54    NC                enabled (pull-down)      low IN
 GPIO 55    NC                enabled (pull-down)      low IN
 GPIO 62    VCAMC_1.2V        enabled (Pull-down)      low out
 GPIO 63    NC                enabled (pull-down)      low IN
 */
// GPIO Function Select --------------------------------------
val = readl(ADDR_SYSCFG_IOCR0);
val |=  ( (SYSCFG_IOCR0_DIGMIC_GPIO6362_MUX)
        | (1 << 27) | (3 << 15) | (1 << 14) | (1 << 13) | (1 << 12));
writel(val, ADDR_SYSCFG_IOCR0);
...
// GPIO I/O direction     -----------------------------------------
// I/O type : output pin (GPIO 62/53/52)
//          : input  pin (GPIO 63/55/54)
val = readl(ADDR_GPIO_IOTR3);
val &= ~((3 << 30) | (3 << 28) | (3 << 14) | (3 << 12) | (3 << 10) | (3  << 8));
val |=  ((2 << 28) | (2 << 10) | (2  << 8));   // GPIO 62/53/52 (output)
writel(val, ADDR_GPIO_IOTR3);

// Pull Enable  ----------------------------
val = readl(ADDR_GPIO_GPIPEN1);
val |= ((1 << 31) | (1 << 30) | (0xF << 20));  // Enable (GPIO 63/62/55/54/53/52)
writel(val, ADDR_GPIO_GPIPEN1);

// Pull Down/Up  ----------------------------
val = readl(ADDR_GPIO_GPIPUD1);
val &= ~((1 << 31) | (1 << 30) | (0xF << 20)); // Pull Down (GPIO 63/62/55/54/53/52)
writel(val, ADDR_GPIO_GPIPUD1);

// Set GPIO defaulit value to low  ----------------------------
val = readl(ADDR_GPIO_GPOR1);
val &= ~((1 << 30) | (0xF << 20));    // Low (GPIO 62/55/54/53/52)
writel(val, ADDR_GPIO_GPOR1);




#if 1    // If you want to enable pull resistors as pull-down
 // Enable pull-up or pull-down --------------------------------
 val = readl(ADDR_GPIO_GPIPEN1);    
 val |=  ( (0x3 << 30) | (0xF << 20) );    // Enabled (GPIO 63/62/55/54/53/52)
 writel(val, ADDR_GPIO_GPIPEN1);
 
 // Set Pull-down  --------------------------------------------
 val = readl(ADDR_GPIO_GPIPUD1);
 val &= ~( (0x3 << 30) | (0xF << 20) );    // Pull down (GPIO 63/62/55/54/53/52) 
 writel(val, ADDR_GPIO_GPIPUD1);
#endif
	...
	case SYSCFG_SDHC3:
		if (op == SYSCFG_INIT) {
			...
			printk("SDIO3 DS is set to 12mA\n");
			val = readl(ADDR_SYSCFG_IOCR4);
...
static void amazing_init_gpio(void)
{
/* +++ H/W req */
#define ADDR_GPIO_GPIPUD0 (HW_GPIO_BASE + 0x028) //0x088CE028 GPIO 0 - 31
#define ADDR_GPIO_GPIPUD1 (HW_GPIO_BASE + 0x02c) //0x088CE02C GPIO 32 - 63
...
#define GPIPUD_PULL_DOWN(GPIO) (~(1<<(GPIO%32)))
				...
				writel(readl(ADDR_GPIO_IOTR0)&(~(3<<IOTR_GPIO(4))),ADDR_GPIO_IOTR0);
				...
				writel(readl(ADDR_GPIO_GPIPEN0)|GPIPEN_PULL_EN(4),ADDR_GPIO_GPIPEN0);
				...
				writel(readl(ADDR_GPIO_GPIPUD0)&GPIPUD_PULL_DOWN(4),ADDR_GPIO_GPIPUD0);
