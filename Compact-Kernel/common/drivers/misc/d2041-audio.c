...
#define D2041_CHIP_ID_OF_AD	0x40
#define D2041_CHIP_ID_OF_AE	0x50
...
int d2041_audio_hs_poweron1(bool on)
{
    ...
    dlg_info("d2041_audio_hs_poweron1 on=%d speaker_power=%d \n", on,dlg_audio->IHFenabled);     
    ...
    if(on) 
    {
		...
		if (!dlg_audio_sleep) {
			ret |= audio_write(D2041_HP_L_CTRL_REG,0xF0);
			ret |= audio_write(D2041_HP_R_CTRL_REG,0xF0);

			ret = audio_write(D2041_HP_L_GAIN_REG,0x00);
			ret = audio_write(D2041_HP_R_GAIN_REG,0x00); 

			ret = audio_write(D2041_HP_NG1_REG,0x10);
			mdelay(30);
		}
		...
		ret = audio_write(D2041_HP_NG1_REG, dlg_set_ng1);
		ret = audio_write(D2041_HP_NG2_REG, dlg_set_ng2);
	...
	else
	{
		...
		ret = audio_write(D2041_HP_NG1_REG,0x11); 
		ret = audio_write(D2041_HP_NG2_REG,0x07);    
		mdelay(200);
...
// For AD silicon
int d2041_audio_hs_poweron_previous_ver(bool on)
{
	int ret = 0;
	u8 regval;

	dlg_info("d2041_audio_hs_poweron_previous_ver on=%d speaker_power=%d \n", on,dlg_audio->IHFenabled);     
	...
	if(on) 
	{
		...
		ret = audio_write(D2041_HP_NG1_REG,0x00);  // set gain to -15 dB
		ret |= audio_write(D2041_HP_NG2_REG,0x00);
...
int d2041_audio_hs_poweron(bool on)
{
    ...
    {
		dlg_info("d2041_audio_hs_poweron CHIPID = 0x%x \n", d2041_chip_id);

		if ( D2041_CHIP_ID_OF_AE <= d2041_chip_id) {
        d2041_audio_hs_poweron1(on);
		} else {
			d2041_audio_hs_poweron_previous_ver(on);
		}
    }
    return 0;
}
...
int d2041_audio_hs_set_gain(enum d2041_audio_output_sel hs_path_sel, enum d2041_hp_vol_val hs_gain_val)
{
    ...
    dlg_info("d2041_audio_hs_set_gain path=%d hs_pga_gain=0x%x \n", hs_path_sel, hs_gain_val);
...
int d2041_audio_hs_ihf_poweron1(void) //speaker on/off
{
   ...
   //     audio_write(D2041_LDO_AUD_MCTL_REG, 0x44); //AUD_LDO on
    ...
    ret |= audio_write(D2041_SP_CFG1_REG,0x00); 
    ret |= audio_write(D2041_SP_CFG2_REG,0x00); 
    //ret |= audio_write(D2041_SP_NG1_REG,0x0F); 
    //ret |= audio_write(D2041_SP_NG2_REG,0x07); 
    ret |= audio_write(D2041_SP_NON_CLIP_ZC_REG,0x00);
    ret |= audio_write(D2041_SP_NON_CLIP_REG,0x00); 
    ret |= audio_write(D2041_SP_PWR_REG,0x00);
    ret |= audio_write(D2041_BIAS_CTRL_REG,0x06); 
...
int d2041_audio_hs_preamp_gain(enum d2041_preamp_gain_val hsgain_val)
{
    ...
    dlg_info("d2041_audio_hs_preamp_gain hs_pre_gain=0x%x \n",hsgain_val);
...
int d2041_set_ihf_noise_gate(u16 regval)
{
    ...
    dlg_info("d2041_set_ihf_noise_gate regval=%d \n",regval);
...
int d2041_set_ihf_none_clip(u16 regval)
{
    ...
    dlg_info("d2041_set_ihf_none_clip regval=%d \n",regval);
...
static int d2041_audio_probe(struct platform_device *pdev)
{
	...
	d2041_chip_id = audio_read(D2041_CHIPID_REG);

	if ( D2041_CHIP_ID_OF_AE <= d2041_chip_id) {
		audio_write(D2041_PREAMP_B_CTRL1_REG,0x35);
		audio_write(D2041_CP_CTRL_REG,0xCD);
	}
...
static int d2041_audio_resume(struct device *dev)
{
	...
	if (D2041_CHIP_ID_OF_AE <= d2041_chip_id) {
		audio_write(D2041_PREAMP_B_CTRL1_REG,0x35);
		audio_write(D2041_CP_CTRL_REG,0xCD);
		audio_write(D2041_HP_L_CTRL_REG,0x70); 
		audio_write(D2041_HP_R_CTRL_REG,0x70); 
	}
