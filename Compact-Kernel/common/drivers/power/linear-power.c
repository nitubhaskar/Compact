...
static int max8986_get_batt_level(struct max8986_power *max8986_power)
{
	...
	* average */
	...
	max8986_power->batt_level_adc_avg = max8986_power->batt_level_adc_avg+COMPENSATION_VALUE;
...
#ifdef  CONFIG_BOARD_AMAZING
		if( ((check_func_called_by_cp==true) && (cnt_func_called>=30)) ||( init_batt_lvl_interval<=5) || ((check_func_called_by_cp==true) && (max8986_power->charging_status ==  POWER_SUPPLY_STATUS_FULL)))
#else
