...
//	msleep(100);	
	...
	if (device_create_file(firmware_dev, &dev_attr_read_idac) < 0)
		pr_err("Failed to create device file(%s)!\n", dev_attr_read_idac.attr.name);	
	if (device_create_file(firmware_dev, &dev_attr_read_global_idac   ) < 0)
...
int firm_update( void )
{
	...
	{
		if((( HW_ver == TSP_HW_VER1 )&&(touch_sw_ver<TSP_SW_VER1))||(( HW_ver == TSP_HW_VER2 )&&(touch_sw_ver<TSP_SW_VER2))||(( HW_ver == TSP_HW_VER3 )&&(touch_sw_ver<TSP_SW_VER3)))
...
retry_read_iDAC:
	...
	i2c_addr_check = 0x01;
	tsp_i2c_read( i2c_addr_check, buf3, sizeof(buf3));
