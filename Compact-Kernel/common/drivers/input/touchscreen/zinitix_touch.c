...
#include "./ZinitixFW/zinitix_touch_bt4x3_firmware_R005.h"
...
void firmware_touch_ctrl_regulator(int on_off)
{
	if(on_off==TOUCH_ON)
	{
		regulator_set_voltage(touch_regulator_3_3,3300000,3300000);
		regulator_enable(touch_regulator_3_3);

	}
	else
	{
		regulator_disable(touch_regulator_3_3);
	}
}
...
void touch_ctrl_regulator(int on_off)
{
	if(on_off==TOUCH_ON)
	{
		...
		regulator_enable(touch_regulator_3_3);
               udelay(100);
		regulator_set_voltage(touch_regulator_2_9,2900000,2900000);
		regulator_enable(touch_regulator_2_9);

	}
	else
	{
		regulator_disable(touch_regulator_2_9);
               udelay(100);
		regulator_disable(touch_regulator_3_3);
	}
}
		...
		const struct i2c_device_id *i2c_id)
{
	...
	if (ret) {
		printk(KERN_ERR "[zinitix_touch] unable to request gpio.(%s)\r\n",
			touch_dev->input_dev->name);
...
static ssize_t read_touchkey(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	for (j = 0; j < misc_touch_dev->cap_info.y_node_num; j++) {
		if ((j == 1) ||  (j == 8)) {
			if ((raw_data[misc_touch_dev->cap_info.x_node_num-1][j] <  CAL_MIN_NUM) || (raw_data[misc_touch_dev->cap_info.x_node_num-1][j] > CAL_MAX_NUM))
			{
                          printk("[zinitix3] : returned by -> raw_data[%d][%d] %5d \n", misc_touch_dev->cap_info.x_node_num-1, j, raw_data[misc_touch_dev->cap_info.x_node_num-1][j]);
				written_bytes += sprintf(buf+written_bytes, "FAIL%5d", raw_data[misc_touch_dev->cap_info.x_node_num-1][j]) ;							  
			}
			else
				written_bytes += sprintf(buf+written_bytes, "PASS%5d", raw_data[misc_touch_dev->cap_info.x_node_num-1][j]) ;		
